...
bool Parser::SaveTokenIT()
{
	savedtokenIT = tokenIT;

	return true;
	/*std::cout << "Token : " << (*tokenIT)->getTokenName() << std::endl;*/
}

std::vector<Token *>::iterator Parser::SaveTokenIT2()
{

	//savedtokenIT = tokenIT;
	std::vector<Token *>::iterator abc = tokenIT;
	return abc;
}

bool Parser::BackTrack()
{
	tokenIT = savedtokenIT;
	return true;
}

bool Parser::BackTrack(std::vector<Token *>::iterator abc)
{
	tokenIT = abc;
	return true;
}
...
Token* Parser::savedToken()
{
	if (savedtokenIT == tokenVector->end())
	{
		return NULL;
	}
...
bool Parser::parseProgram()
{
	...
	if (tokenIT != tokenVector->end())
	{
		...
		// while (tokenIT != tokenVector->end())
		// {
		// 	std::cout << (*tokenIT)->getTokenName()<<" is "<< (*tokenIT)->getStringID()<<std::endl;
		// 	++tokenIT;
		// }
		return temp;
	}
