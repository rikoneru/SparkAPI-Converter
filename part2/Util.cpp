...
std::pair<bool, int> Util::isSymbol(std::string::iterator pos)
{
	...
	case '.':
		return std::pair<bool, int>(true, Token::SYMTYPE_FULL_STOP);
	case ')':
	...
	case '%':
		return std::pair<bool, int>(true, Token::SYMTYPE_MOD);
	case '/':
...
std::pair<bool, int> Util::isDoubleSymbol(std::string::iterator pos)
{
	...
	case '=':
		...
		return std::pair<bool, int>(true, Token::SYMTYPE_DOUBLE_EQUAL);
	    	...
	    	return std::pair<bool, int>(true, Token::SYMTYPE_IMPLIES);
	case '<':
		if (*(pos + 1) != '=')
		{
			return std::pair<bool, int>(false, Token::SYMTYPE_INVALID);
...
// std::pair<bool, int> Util::isCollectOrMap(std::string::iterator pos)
// {
// 	if(*pos == '.')
// 	{
// 		if(*(pos+1) == 'c')
// 		{
// 			if(*(pos+2) == 'o' && *(pos+3) == 'l' && *(pos+4) == 'l' && *(pos+5) == 'e' && *(pos+6) == 'c' && *(pos+7) == 't')
// 				return std::pair<bool, int>(true, Token::IDTYPE_RESERVEDWORD);
// 		}
// 	}
// }
