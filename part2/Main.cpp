...
{   int c = 0;
	...
	Scanner scan(argv[1], isOpen);

	//std::cout << isOpen << std::endl;
	if(!isOpen)
	{
		std::cout << "ERROR: The given input file does not exist" << std::endl;
		return EXIT_FAILURE;	
	}
	...
	while (scan.hasNextToken())
	{
		...
		else 
		{
			c++;
			parse.addToken(t);
			//std::cout<<t->getTokenName()<<" is "<<t->getStringID()<<" "<<c<<std::endl;
		}
	...
	{
		std::cout<<"The given input program follows all the syntax rules"<<std::endl;
	}
}
